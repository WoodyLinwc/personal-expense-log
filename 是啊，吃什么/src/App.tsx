...
export default function App() {
  ...
  const frequentEntries = useMemo(() => {
    const counts = new Map<string, { entry: Omit<RecordItem, 'id' | 'createdAt'>, count: number, lastUsed: number }>();
    Object.values(records).flat().forEach((r) => {
      if (!r) return;
      const key = `${r.description}|${r.cost}|${r.category || 'food'}|${r.customColor || ''}`;
      if (!counts.has(key)) {
        counts.set(key, { 
          entry: { description: r.description, cost: r.cost, category: r.category || 'food', customColor: r.customColor }, 
          count: 0, 
          lastUsed: 0 
        });
      }
    ...
    return Array.from(counts.values())
      ...
      .slice(0, 8)
      .map(x => x.entry as { description: string; cost: number; category: any });
  }, [records]);
      ...
      <main className="flex-1 flex flex-col z-10">
        <header className="h-24 px-10 flex items-center justify-between border-b border-[rgba(0,0,0,0.05)] shrink-0 bg-[#FAF9F6]/80 backdrop-blur-md">
          <div className="flex items-baseline gap-4">
            <h1 className="font-serif text-3xl sm:text-5xl tracking-tighter">是啊，吃什么</h1>
            <span className="text-[10px] sm:text-sm font-semibold uppercase tracking-widest opacity-40 hidden sm:inline-block">Life Expense Log</span>
          </div>
          <div className="flex items-center gap-6 sm:gap-8">
            ...
            <div className="flex items-center gap-6 hidden md:flex">
                <div className="text-right cursor-pointer group" onClick={() => setIsTotalModalOpen(true)}>
                  <p className="text-[10px] uppercase tracking-widest opacity-50 font-bold group-hover:opacity-100 transition-opacity">Total Spent <span className="opacity-0 group-hover:opacity-100">&rarr;</span></p>
                  <p className="font-serif text-2xl italic opacity-50 group-hover:opacity-100 transition-opacity">¥{lifetimeTotal.toFixed(2)}</p>
                </div>
                <div className="w-[1px] h-8 bg-black/10"></div>
                <div className="text-right">
                  <p className="text-[10px] uppercase tracking-widest opacity-50 font-bold">Monthly Spent</p>
                  <p className="font-serif text-3xl italic">¥{monthTotal.toFixed(2)}</p>
                </div>
            </div>
          </div>
        </header>
          ...
          <div className="flex-1 flex flex-col bg-[#FAF9F6]/60 backdrop-blur-md border border-[rgba(0,0,0,0.08)] rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="flex-1 grid grid-cols-7 auto-rows-fr overflow-y-auto min-h-0 bg-transparent">
              {monthDays.map((day, idx) => {
                ...
                const colorRecords = dailyRecords.filter(r => !r.description && (!r.cost || r.cost === 0) && r.customColor);
                const dayBgClass = colorRecords.length > 0 ? CUSTOM_COLORS[colorRecords[colorRecords.length - 1].customColor!].split(' ')[0] : null;
                const displayRecords = dailyRecords.filter(r => r.description || (r.cost && r.cost > 0) || !r.customColor);
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2 relative z-10">
                       <span className={`font-bold text-sm ${day.isCurrentMonth ? '' : 'italic opacity-60'} ${isToday ? 'bg-black text-white w-6 h-6 flex items-center justify-center rounded-full text-xs' : ''}`}>
                         {isToday ? day.date.getDate() : String(day.date.getDate()).padStart(2, '0')}
                       </span>
                    </div>

                    <div className="flex flex-col gap-1 overflow-y-auto scrollbar-none flex-1 min-h-0 z-10 w-full relative">
                      {displayRecords.map((r) => {
                          const c = (r.customColor && CUSTOM_COLORS[r.customColor]) || CATEGORY_COLORS[r.category || 'food'] || CATEGORY_COLORS.food;
                          return (
                            <div
                              key={r.id}
                              onDoubleClick={(e) => {
                              ...
                              title={r.cost > 0 ? `${r.description} ¥${r.cost}` : r.description}
                            >
                              <span className="whitespace-nowrap overflow-hidden text-ellipsis font-medium flex-1 text-left">
                                {r.description || '\u00A0'}
                              </span>
                              {r.cost > 0 && <span className="shrink-0 font-bold">¥{r.cost}</span>}
                            </div>
                          );
                      })}
                    </div>
